...
from gheimdall import passwd, utils
import ldap
...
class LdapPasswdEngine(passwd.BaseSyncPasswdEngine):
  ...
  def _prepare(self, config):
    ...
    self.ldap_uri = config.get('apps.ldap_uri')
    self.ldap_basedn = config.get('apps.ldap_basedn')
    self.ldap_filter = config.get('apps.ldap_filter')
    self.ldap_rootdn = config.get('apps.ldap_rootdn')
    self.ldap_rootpw = config.get('apps.ldap_rootpw')
    self.ldap_passwd_hash_style = config.get('apps.ldap_passwd_hash_style')
  ...
  def _resetLocalPassword(self, user_name, new_password):
    hashed_pass = utils.hashPassword(new_password, self.ldap_passwd_hash_style)
    ldapHandle = None
    try:
      ldapHandle = ldap.initialize(self.ldap_uri)
      ldapHandle.simple_bind_s(who=self.ldap_rootdn, cred=self.ldap_rootpw)
